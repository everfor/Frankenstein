...
{
	public:
		PointLight(BaseLight& init_base, glm::vec3& init_pos, float init_constant = 0.0f, float init_linear = 0.0f, float init_exp = 0.0f) :
			base(init_base), position(init_pos), constant(init_constant), linear(init_linear), exponent(init_exp) {};
		virtual ~PointLight() {};
